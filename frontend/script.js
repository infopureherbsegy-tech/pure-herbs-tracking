...
async function trackShipment() {
    ...
    try {
        ...
        // Open the same order-style tab, but in a simplified tracking mode.
        // It intentionally shows only the essential shipment information.
        const trackingOrderId = shipment.orderId || order;
        if (trackingOrderId) {
            await openOrderModal(trackingOrderId, true);
        }
...
async function getRegisteredUsers() {
    const data = await apiRequest("/registered-users");
    return data.users || [];
}
...
async function handleLogin() {
    ...
    try {
        ...
        sessionStorage.setItem(
            SESSION_STORAGE_KEY,
            JSON.stringify({ role: "customer", email: customer.email, name: customer.name })
        );
        banner.classList.add("hidden");
        clearAuthFields();
        await enterCustomer(customer);
    } catch (error) {
        banner.textContent = error.message || "Account not found or password is incorrect.";
...
async function handleSignup() {
    ...
    try {
        ...
        setFieldState("signupEmail", "signupEmailError", error.message || "Could not create account.");
    }
}
...
async function handleForgotPassword() {
    ...
    try {
        ...
        setFieldState("forgotEmail", "forgotEmailError", error.message || "No customer account was found with this email.");
        return;
    }
...
let publicTrackingView = false;

async function renderRegisteredUsersTable() {
    ...
    try {
        ...
        body.innerHTML = users.length ? users.map(user => `
            <tr>
                <td class="mono-strong">${user.name}</td>
                <td>${user.email}</td>
                <td>${user.createdAt ? new Date(user.createdAt).toLocaleString() : "—"}</td>
                <td>${user.lastLogin ? new Date(user.lastLogin).toLocaleString() : "Never"}</td>
                <td>${Number(user.loginCount || 0)}</td>
                <td><span class="tag ${user.status === "Inactive" ? "" : "tag-active"}">${user.status || "Active"}</span></td>
            </tr>
        `).join("") : `<tr><td colspan="6"><div class="empty-state">No registered customer accounts yet.</div></td></tr>`;
    } catch (error) {
        body.innerHTML = `<tr><td colspan="6"><div class="empty-state">${error.message || "Unable to load registered users."}</div></td></tr>`;
    }
}
...
async function openOrderModal(orderId, isPublicTracking = false) {
    ...
    publicTrackingView = Boolean(isPublicTracking);
                                ...
                                (substep, subIndex) => {
                                    ...
                                    return `
                                        <div class="m-substep ${subClass}">
                                            <span class="m-substep-number">${subIndex + 1}.</span>
                                            <span>${substep}</span>
                                        </div>
                                    `;
                                }
    ...
    if (publicTrackingView) {
        const modal = document.querySelector("#orderModalOverlay .order-modal");
        if (modal) modal.classList.add("tracking-simple");

        const eyebrow = document.querySelector("#orderModalOverlay .m-eyebrow");
        if (eyebrow) eyebrow.textContent = "Shipment Tracking";

        const advancedBox = document.getElementById("mAdvancedDetails");
        if (advancedBox) advancedBox.innerHTML = "";

        const documentsBox = document.getElementById("orderDownloadDocuments");
        if (documentsBox) documentsBox.style.display = "none";
        ...
        const stepper = document.getElementById("mStepper");
        if (stepper) {
            stepper.innerHTML = simpleSteps.map(([label, threshold], index) => {
                const isDone = pct >= threshold && index < simpleSteps.length - 1;
                const isCurrent =
                    (index === 0 && pct < 25) ||
                    (index === 1 && pct >= 25 && pct < 60) ||
                    (index === 2 && pct >= 60 && pct < 100) ||
                    (index === 3 && pct >= 100);

                const cls = isCurrent ? "current" : (isDone ? "done" : "");
                return `
                    <div class="m-step tracking-simple-step ${cls}">
                        <div class="m-step-marker"><div class="dot">${isDone ? "✓" : index + 1}</div></div>
                        <div class="m-step-content">
                            <div class="m-step-head">
                                <div class="m-step-title">${label}</div>
                                <span class="m-step-state">${isCurrent ? "Current" : (isDone ? "Completed" : "Upcoming")}</span>
                            </div>
                        </div>
                    </div>
                `;
        ...
        const trackingInfo = Object.values(PUBLIC_TRACKING).find(
            item => item && item.orderId === order.id
        ) || {};

        const alertBox = document.getElementById("mAlert");
        if (alertBox) alertBox.innerHTML = "";
        if (alertBox && trackingInfo.message) {
            alertBox.innerHTML = `<div class="m-alert">${escAdvanced(trackingInfo.message)}</div>`;
        }
    } else {
        const modal = document.querySelector("#orderModalOverlay .order-modal");
        if (modal) modal.classList.remove("tracking-simple");

        const documentsBox = document.getElementById("orderDownloadDocuments");
        if (documentsBox) documentsBox.style.display = "";
        ...
        if (box) {
            ...
            box.innerHTML = `
                ...
                <div class="m-docs" style="margin-top:18px">
                  ...
                  ${!publicTrackingView ? `<div style="display:flex;gap:8px;flex-wrap:wrap;margin:10px 0 12px">
    ...
    const downloadDocs = document.getElementById("orderDownloadDocuments");
    if (downloadDocs) {
        downloadDocs.style.display = publicTrackingView ? "none" : "";
    }
